    ...
    while summa_deltaE > DeltaUmin 
        ...
        for y = 1:h
            for x = 1:w
                ...
                neighbourLabels = mrf.classmask(max([y-1 1]) : min(y+1,mrf.imagesize(1)),   max([x-1 1]) : min(x+1, mrf.imagesize(2)));
                
                
                
                curr_posterior = mrf.logProbs{currLabel}(y, x);
                curr_prior = sum(sum(-beta .* (neighbourLabels == currLabel) + beta .* (neighbourLabels ~= currLabel & neighbourLabels ~= 0)));
                curr_prior = curr_prior + beta; % because the current pixel was also in it
                ...
                tryal_prior = sum(sum(-beta .* (neighbourLabels == tryalLabel) + beta .* (neighbourLabels ~= tryalLabel & neighbourLabels ~= 0)));
                tryal_prior = tryal_prior - beta; % because the current pixel was also in it
